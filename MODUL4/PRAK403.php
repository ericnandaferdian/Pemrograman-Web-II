...
function hitungTotalSKS($mataKuliah) {
    $total = 0;
...
foreach ($students as $index => $student) {
    $totalSKS = hitungTotalSKS($student["Mata Kuliah"]);
    $students[$index]["Total SKS"] = $totalSKS;
    $students[$index]["Keterangan"] = $totalSKS < 7 ? "Revisi KRS" : "Tidak Revisi";
}
?>
...
<table>
    ...
    <?php foreach ($students as $student): ?>
        <tr>
            ...
            <?php foreach ($student["Mata Kuliah"] as $i => $mk): ?>
                <?php if ($i == 0): ?>
